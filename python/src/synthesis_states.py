...
So the pipeline precomputes the small, LINEAR set of states the toggle can
reach -- the full corpus plus each contested (default-hidden) document removed
-- and the frontend swaps to the matching precomputed set with no live LLM call
at view time. The contested set is exactly `defaultHiddenDocTypes` from the
country config, so a country that hides one document (Panama / ENR) precomputes
just two states: full and minus-that-document.
...
def precompute_hidden_states(default_hidden: list[str]) -> list[list[str]]:
    """The hidden-doc sets to precompute storylines for.

    Returns the full corpus (``[]``) plus one state per default-hidden document
    (that document removed). When more than one document is hidden by default it
    also adds the combined default view, so the initial page load always has an
    exact precomputed match. Deduplicated by canonical key (order preserved) and
    capped by ``MAX_PRECOMPUTE_DOCS``.
    """
