...
async def test_classify_with_mock_llm(sample_targets, sample_nbs_categories):
    ...
    # Return "1" for first 5, "0" for rest
    responses = ["1"] * 5 + ["0"] * 5
...
async def test_alignment_with_mock_llm(sample_targets, sample_classifications):
    ...
    # Create a mix of alignment and contradiction responses
    mock_responses = [
    ...
    # Check that contradiction types are extracted
    ...
    # Check that at least one has contradictionType set
...
async def test_quantitative_enforces_number_check():
    ...
    # LLM says quantitative, but the text has no numbers
    llm_response = json.dumps([
        {
            "targetId": "NAP_1",
            "isQuantitative": True,
