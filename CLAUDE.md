...
- **Level 3 — Implementation progress tracking.** The "are these policies actually being implemented?" layer. Current implementation lives in `src/components/viz/implementation-coverage.tsx` (lead-with-gap mitigation × IPCC sector + adaptation × APNDC goal coverage tables). Replaced the earlier `sector-scorecard.tsx`.
...
- Prototype methodology cleanup leans freeze + reframe over revert. Revert is reserved for items that actively mislead even within prototype framing. Why: reverts have social cost and time cost; reframing closes the same defensibility gap because most prototype concerns are about *implied precision/certainty*, not the existence of the metric. Tightening the page-level prototype framing alone may resolve several flagged items.
