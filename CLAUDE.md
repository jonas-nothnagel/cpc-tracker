...
- Self-explanatory visuals, no text walls. Never caption a chart with how to read it. Label marks in place, reveal detail on interaction, and keep a legend only when colour cannot be labelled directly. Method and provenance belong on the How it works page or in a walkthrough; AI labels stay as short tags. Never fade text to fit. Copy follows the team's register (proposal, decks, results 2-pager, UNDP Editorial Style Manual), not generic assistant phrasing. Why: in the Sep 2026 brief review, the project lead skipped the method text and could not decode two captioned visuals.
...
- Adaptation alignment uses the same 5-category scale as mitigation, with a prompt-level instruction in `measure_align.py` not to penalize adaptation actions for missing CO2e numbers. Why: adaptation reporting is less structured (Apr 2026 Mongolia call); the asymmetry is prompt-level, not schema-level, so treat it as fragile.
...
- Pathway-style suggestions (process pointers: boundary review, joint M&E, coordination, indicator alignment, triage of flagged pairs) are allowed only in chat replies, insight callouts, and theme drill-down drawers (under a labeled "AI-suggested starting point" heading with a caveat), anchored to visible evidence and hedged ("could", "may", "worth a closer look"); they may name specific documents and targets, never ministries, agencies, or individual actors; never "should" or "must", no extended narratives. All other static surfaces (headline cards, KPI tiles, status badges, card and row faces) stay factual-only. Why: stakeholders repeatedly asked for practical high-level pointers (May-June 2026); a labeled hedged starting point inside a review surface is decision-support, not prescription.
