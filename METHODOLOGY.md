...
## Step 6b: NR7 Biodiversity Self-Report (Level 3, conditional, no LLM in the input)

**Purpose:** Bring the country's 7th National Report to the Convention on Biological Diversity (NR7) into the implementation lens beside the BTR, as the country's own statement of progress on its national biodiversity targets. This is the second self-reported lens; like the BTR it is not the full implementation picture.

**Input (no LLM):** `python/scripts/fetch_nr7_ort.py` pulls the report as structured public data from the CBD Online Reporting Tool's API (`python/src/nr7_ort.py`), replacing the March 2026 PDF scrape. Three exports are kept verbatim, raw CSVs saved for provenance under `python/data/external/nr7_ort/{iso3}/`:

1. **Section III** (one row per national target): the four narratives kept separate (Main Actions, Progress, Key Challenges, Action Effectiveness), the six-level "Level of Progress" folded onto four statuses (`on_track`, `limited`, `no_progress`, `unknown`; the raw wording is preserved), and the **GBF global target(s)** the country filed each national target under, with the CBD's heading verbatim (`gbfTargets`, added 2026-09-11). The national target is matched to the corpus NBSAP target by text similarity (threshold 0.85), never hand-coded.
2. **Headline, component and national indicators**: indicator × year × disaggregation series with units.
3. **Binary indicators** (the GBF questionnaire): yes / partially / under development / no answers per question, wording from the tool.

**Output:** `python/data/external/nr7_{iso3}.json`, read directly by the dashboard. Everything the biodiversity view computes from it (rating-vs-evidence cross-checks, grouping by GBF target) is arithmetic on the report's own statements.

**Joined to Level 1:** the view ranks the national targets the report rates behind schedule by how many policy targets in other documents Step 5 rated HIGH against the NBSAP target each restates (`policyLinksByNbsap`), and shows which documents and counterparts those are. These links are the pipeline's AI-estimated target × target alignment and are labelled as such; they say the texts were judged coherent, not that one delivers or funds the other.

**Optional LLM step (opt-in, uncalibrated):** `python/scripts/run_nr7_alignment.py` (`python/src/nr7_align.py`) turns each national target's Main Actions Summary into a pseudo-target and scores it against every policy target on the Step 5 scale, writing `nr7_pseudo_targets.json` and `nr7_alignment.json`. Its flagged pairs are **not** shown as review items until the run is calibrated (see the CALIBRATION TODO in `nr7_align.py`); the dashboard uses them only for the coverage dot-map and the "reported action and NBSAP target side by side" link.
