...
- `Nr7ReportsStrip` at the top of the Implementation slide's evidence area:
  a BTR card (climate) and an NR7 card (biodiversity) with headline numbers.
  The NR7 card ends with up to three "worth a closer look" lines and the
...
| Rule | Fires when | On the card |
|---|---|---|
| `ratingVsAnswers` | rated on track, at least 3 scale answers, half or more of them "under development" or "no" | yes |
| `flatWhileOnTrack` | rated on track, a target-specific series (indicator on at most 3 targets) with at least 3 numeric points is unchanged (within 1% of its first value) | yes |
| `unknownWithData` | rated unknown while the report carries values for a target-specific indicator | yes |
| `reachWhileNoChange` | no significant change, policy reach at or above the 75th percentile of matched targets | yes |
| `sharedIndicatorDeclining` | an indicator on 4 or more targets whose series with 3+ points all fall; once per indicator | **no**, drawer only until the reading of the funding series is reviewed (decision 2026-09-09) |

The card takes one signal per rule in priority order (rating vs answers,
unknown with data, flat while on track, reach while no change), then fills
to three. Deterministic.
...
`briefing-panels.tsx`; the `nr7Report` / `btrCoverage` memos, `openNr7Report`
and the two prop groups in `index.tsx`; the import, mount and three props in
...
files; the `nr7-report` line in `src/lib/analytics/sections.ts`. Restore
`briefing.implementation.source.label` to "Evidence from" (and its
translations) if the plain label is preferred. Then
