...
1. **Headline = the finding for the report on screen, with its
   ...
   actions may work against targets in its other plans." Biodiversity: "In
   {k} places, {country}'s biodiversity report rates a target one way while
   its own evidence points another." (variants for nothing flagged and for a
   match not computed).
2. **Body = what was found, in two plain sentences**, templated from the
   ...
   in brackets) most of the targets sit in. Biodiversity: what the report
   gives per national target, and that these are the places it does not
   agree with itself; "No AI is involved." Under 35 words in all three
...
3. **"Where to start"** (`WhereToStart` in `index.tsx`, the theme drawer's
   ...
   "AI-estimated review prompts, not findings." Biodiversity: open a row,
   then settle which side is right before the next report; caveat "Nothing
   on this tab is AI-generated." Tool guidance and a hedged process pointer,
   never a policy suggestion, no actors named.
