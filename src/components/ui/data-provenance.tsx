...
> = {
  "user-uploaded": {
    label: "User-uploaded data",
    tone: "bg-gray-50 text-[var(--undp-gray)] border-gray-200 hover:bg-gray-100",
    icon: (
  ...
  mixed: {
    label: "AI-inferred from uploaded data",
    tone:
      "bg-[var(--undp-blue)]/8 text-[var(--undp-blue)] border-[var(--undp-blue)]/30 hover:bg-[var(--undp-blue)]/12",
    icon: (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none" aria-hidden>
  ...
  return (
    ...
    >
      <button
        ...
        className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded border transition-colors cursor-pointer ${meta.tone}`}
      >
        {meta.icon}
      ...
      {open && (
        ...
        >
          ...
          {method != null && (
            <ProvenanceSection
              title={origin === "user-uploaded" ? "Aggregation" : "AI method"}
            >
              <p>{method}</p>
            </ProvenanceSection>
          )}
