...
function uploadRequest(file: File): NextRequest {
  ...
  form.set("file", file);
  // The route only calls request.formData(); duck-type it to avoid FormData
  // serialization quirks in the test environment.
  return { formData: async () => form } as unknown as NextRequest;
}

describe("POST /api/parse-btr size guard", () => {
  ...
  it("still rejects a non-Excel file with 400 (before the size check)", async () => {
